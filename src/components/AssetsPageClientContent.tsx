...
import GenerationMixWrapper from '@/components/GenerationMixWrapper';
import AssetControlPanel from '@/components/AssetControlPanel';
import { useState } from 'react';

// Client component wrapper for page content to handle state
export default function AssetsPageClientContent({ initialAssets }: { initialAssets: any[] }) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/20">
                            ...
                            <p className="text-2xl font-bold text-white">{initialAssets.filter(a => a.status === 'online').length}</p>
                        </div>
                        <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                            <p className="text-sm text-blue-300">{t('capacity.nominal')}</p>
                            <p className="text-2xl font-bold text-white">{initialAssets.reduce((sum, a) => sum + a.capacity, 0)} MW</p>
                        </div>
                        <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                            <p className="text-sm text-amber-300">{t('capacity.current')}</p>
                            <p className="text-2xl font-bold text-white">{initialAssets.reduce((sum, a) => sum + a.output, 0).toFixed(1)} MW</p>
                        </div>
                    </div>
                                ...
                                {initialAssets.map((asset) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="text-sm font-mono text-slate-300 group-hover:text-blue-400 transition-colors">{asset.id}</span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
            ...
            <AssetControlPanel
                ...
                maxOutput={selectedAsset?.capacity}
            />
        </div>
    );
}
